...
class LevelParser{
  constructor(wordBooks){
    this.wordBooks = wordBooks;
  }

  actorFromSymbol(wordBook){
    return wordBook ? this.wordBooks[wordBook] : undefined;
  }

  obstacleFromSymbol(wordBook){
    switch(wordBook) {
      case 'x' : return 'wall';
      case '!' : return 'lava';
  ...
  createActors(plan){
    ...
    mas.forEach((row, y) => {
      row.forEach((cell, x) => {
        if (this.wordBooks && this.wordBooks[cell] && typeof this.wordBooks[cell] === 'function') {
          const actor = new this.wordBooks[cell] (new Vector(x, y));
          if (actor instanceof Actor) {
              actors.push(actor);
          }
        }
